...
public class MembersExample {
    ...
    //or:通过or进行sql语句拼接，通过or（）方法获取criteria对象，并且可以调用andUsernameLike（）添加条件
    //通过or（）方法获取criteria对象，并且可以调用andPhoneLike（）添加条件
    //如果以上的两个条件需要通过or拼接，只需要将以上两步生成的Criteria对象，加入到集合中List<Criterio>
    public Criteria or() {
